...
const Navbar = () => {
  ...
  useEffect(() => {
    const windInterval = setInterval(() => {
      const windX = Math.floor(Math.random() * 3) - 1;
      const windY = Math.floor(Math.random() * 3) - 1;
      setWind([windX, windY]);
    }, 500);
  ...
  return (
    <nav className=" flex justify-between px-[6%]  items-center">
      ...
      <a
        ...
        className="lg:flex xs:hidden gap-1 text-[16px] font-medium  "
      >
      ...
      {isMenuOpen && (
        <div
          className={`lg:hidden transition-transform ease-linear duration-300 rounded-s-[50px] flex flex-col  h-[100vh] bottom-0 top-0 left-[15%] right-0 gap-10 justify-evenly items-center bg-secondary3 fixed text-mainWhite  z-[99999] ${
            isMenuOpen ? "translate-x-[0] " : "translate-x-[100%]"
          }`}
        >
          <button
            onClick={toggleMenu}
            className="xs:flex lg:hidden self-end pe-6 -mt-6"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
